...
def gather_data(nb_samples: int, *, std: float = 0.1, theta=None, rg: npr.Generator = None):
    if rg is None: rg = npr.default_rng()
    if theta is None: theta = [0.5, 3]
...
# Serializable arrays (``sarray``) provide some additional functionality
# when used in a ``modelclass`` especially useful in configuration classes.
# We specifically state that the array is of type integer and should 
# be comparable. When assigning an array to the class, it will 
# be converted automatically. 
from dman.numeric import sarray, sarrayfield
_nb_samples_field = sarrayfield(
    compare=True, as_type=int, default_factory=lambda: (
        np.logspace(1, 3, 20)
    )
)
...
class Configuration:
    ...
    nb_samples: sarray = _nb_samples_field
    nb_repeats: int = 1000
    seed: int = None
...
_data_field = dman.recordfield(
    default_factory=lambda: (
        dman.smdict(store_subdir=False, store_by_key=True)
    ),
    subdir='data', stem='__data', repr=False
)
...
class Experiment:
    ...
    def generate(cls, cfg: Configuration, idx: int, *, verbose: bool = True):
        ...
        res = cls(
            model=cfg.model,
            nb_samples=cfg.nb_samples[idx], 
            nb_repeats=cfg.nb_repeats
        )
        res.data.update({
        ...
        if verbose:
            _iter = tui.track(
                _iter, 
                total=cfg.nb_repeats, 
                description='Executing experiment ...'
            )
        for i in _iter:
